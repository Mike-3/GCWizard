...
*/
void genColorPalette(int color_number, Int8 palette){
	if(color_number < 8)
		return ;
 ...
 * @param palette the color palette
 */
void setDefaultPalette(int color_number, Int8 palette) {
    if(color_number == 4)
    {
 ...
 */
void setDefaultEccLevels(int symbol_number, Int8 ecc_levels){
  ecc_levels = List<ecc_levels>.filled(symbol_number, 0)
}
 ...
 */
void convert_dec_to_bin(int dec, String bin, int start_position, int length)
{
    if(dec < 0) dec += 256;
...
jab_encode createEncode(int color_number, int symbol_number)
{
    ...
    enc.symbol_versions = List<jab_vector2d>.filled(symbol_number);
    ...
    enc.symbols = List<jab_symbol>.filled(symbol_number);
    if(enc.symbols == null)
    {
 ...
 */
int analyzeInputData(jab_data input, int encoded_length)
{
    int encode_seq_length=ENC_MAX;
    String seq = (jab_char *)malloc(sizeof(jab_char)*input.length);
    if(seq == null) {
    ...
    for (int i=0; i < 28; i++)
        temp_switch_mode[i] = (ENC_MAX/2).toInt();
    ...
    var encode_seq = List<int>.filled((curr_seq_counter+1+is_shift), 0);
    if(encode_seq == null)
    {
 ...
 * @return JAB_SUCCESS | JAB_FAILURE
*/
jab_boolean updateMasterMetadataPartII(jab_encode* enc, int mask_ref)
{
 ...
 * @param color_number the number of colors
*/
void getColorPaletteIndex(jab_byte* index, int index_size, int color_number)
{
 ...
 * @return JAB_SUCCESS | JAB_FAILURE
*/
jab_boolean createMatrix(jab_encode* enc, int index, jab_data* ecc_encoded_data)
{
    //Allocate matrix
    ...
    int Nc = log(enc.color_number)/log(2.0) - 1;
		...
		if(!isDefaultMode(enc))
		{
			while(metadata_index < enc.symbols[index].metadata.length)
			{
				...
				for(int j=0; j<nb_of_bits_per_mod; j++)
				{
					...
					{
						color_index += ((int)enc.symbols[index].metadata.data[metadata_index]) << (nb_of_bits_per_mod-1-j);
						metadata_index++;
					}
					else
						break;
            ...
            if (enc.symbols[index].data_map[i]!=0 && written_mess_part<ecc_encoded_data.length)
            {
                ...
                for(int j=0;j<nb_of_bits_per_mod;j++)
                {
                    if(written_mess_part<ecc_encoded_data.length)
                        color_index+=((int)ecc_encoded_data.data[written_mess_part]) << (nb_of_bits_per_mod-1-j);//*pow(2,nb_of_bits_per_mod-1-j);
                    else
                    {
...
*/
void swap_symbols(jab_encode* enc, int index1, int index2)
{
...
jab_code* getCodePara(jab_encode* enc)
{
    ...
    cp.row_height = (int *)malloc(cp.rows * sizeof(int));
    if(!cp.row_height)
    {
		free(cp);
    ...
    cp.col_width = (int *)malloc(cp.cols * sizeof(int));
    if(!cp.col_width)
    {
		free(cp);
...
jab_boolean setMasterSymbolVersion(jab_encode *enc, jab_data* encoded_data)
{
	...
	{
		int level = -1;
		for (int j=(int)enc.symbol_ecc_levels[0]-1; j>0; j--)
		{
...
jab_boolean fitDataIntoSymbols(jab_encode* enc, jab_data* encoded_data)
{
	...
	int capacity[enc.symbol_number];
	int net_capacity[enc.symbol_number];
	...
	for(int i=0; i<enc.symbol_number; i++)
	{
		...
		{
			double prop = (double)net_capacity[i] / (double)total_net_capacity;
			s_data_length = (int)(prop * encoded_data.length);
		}
...
int generateJABCode(jab_encode* enc, jab_data* data)
{
    ...
    int encoded_length;
    int* encode_seq = analyzeInputData(data, &encoded_length);
    if(encode_seq == null)
	{
