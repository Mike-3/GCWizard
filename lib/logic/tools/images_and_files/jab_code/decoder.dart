...
*/
int readColorPaletteInMaster(jab_bitmap matrix, jab_decoded_symbol symbol, Int8 data_map, List<int> module_count, List<int> x, List<int> y)
{
 ...
 * @return JAB_SUCCESS | FATAL_ERROR
*/
int readColorPaletteInSlave(jab_bitmap* matrix, jab_decoded_symbol* symbol, jab_byte* data_map)
{
	//allocate buffer for palette
...
int getNearestPalette(jab_bitmap* matrix, int x, int y)
{
	...
	int px[COLOR_PALETTE_NUMBER], py[COLOR_PALETTE_NUMBER];
...
*/
jab_byte decodeModuleHD(jab_bitmap* matrix, jab_byte* palette, int color_number, jab_float* norm_palette, jab_float* pal_ths, int x, int y)
{
...
*/
void getPaletteThreshold(jab_byte* palette, int color_number, jab_float* palette_ths)
{
	if(color_number == 4)
	{
...
*/
int decodeMasterMetadataPartI(jab_bitmap* matrix, jab_decoded_symbol* symbol, jab_byte* data_map, int* module_count, int* x, int* y)
{
...
*/
int decodeMasterMetadataPartII(jab_bitmap* matrix, jab_decoded_symbol* symbol, jab_byte* data_map, jab_float* norm_palette, jab_float* pal_ths, int* module_count, int* x, int* y)
{
	...
	int color_number = (int)pow(2, symbol.metadata.Nc + 1);
	int bits_per_module = (int)(log(color_number) / log(2));
...
*/
void fillDataMap(jab_byte* data_map, int width, int height, int type)
{
 ...
 * @return JAB_SUCCESS | JAB_FAILURE | DECODE_METADATA_FAILED | FATAL_ERROR
*/
int decodeSymbol(jab_bitmap* matrix, jab_decoded_symbol* symbol, jab_byte* data_map, jab_float* norm_palette, jab_float* pal_ths, int type)
{
#if TEST_MODE
	int color_number = (int)pow(2, symbol.metadata.Nc + 1);
	printf("p1:\n");
	...
	//demask
	demaskSymbol(raw_module_data, data_map, symbol.side_size, symbol.metadata.mask_type, (int)pow(2, symbol.metadata.Nc + 1));
	free(data_map);
#if TEST_MODE
...
void normalizeColorPalette(jab_decoded_symbol* symbol, jab_float* norm_palette, int color_number)
{
