...
EpochTimeOutput DateTimeToUnixTime(DateTimeTZ currentDateTime) {
  if (_invalidUnixDate(gregorianCalendarToJulianDate(currentDateTime.toLocalTime()))) {
    return EpochTimeOutput(
      Error: 'dates_calendar_unix_error',
      timeStamp: 0,
      UTC: currentDateTime.dateTimeUtc,
      local: currentDateTime.toLocalTime(),
    );
  }

  return EpochTimeOutput(
      timeStamp:
      (currentDateTime.toLocalTime().difference(DateTime.utc(1970,1,1)).inSeconds + currentDateTime.timezone.inSeconds).toDouble(),
      UTC: currentDateTime.dateTimeUtc,
      local: currentDateTime.toLocalTime(),
      Error: '');
}
