...
bool isValid(List<List<int>> grid, int row, int col, int number) {
  // Überprüfe die Reihe (horizontal)
  ...
  // Überprüfe die Spalte (vertikal, angepasst an die Dreiecksform)
  for (int i = 0; i < grid.length; i++) {
    if (i > row && col < grid[i].length && grid[i][col] == number) {
      return false;
    }
    if (i < row && col < grid[i].length && grid[i][col] == number) {
      return false;
  ...
  // Überprüfe den Block (3x3-Bereich, angepasst an die Dreiecksform)
  ...
  for (int i = blockRowStart; i < blockRowStart + 3; i++) {
    if (i < grid.length) {
      ...
      for (int j = blockColStart; j < blockColEnd; j++) {
        if (i < grid.length && j < grid[i].length && grid[i][j] == number) {
          return false;
        }
      }
    }
