...
class TetrisPuzzleSolverMT {
    ...
    //SlaveThread[] slaves = new SlaveThread[Runtime.getRuntime().availableProcessors()]; //1 slave thread per core (+1 master thread)
    ...
    int w, h; //cols and rows of the board
    ...
    var ret = List<List<List<int>>>.generate(1, (y) => List<List<int>>.generate(cachedResult.board.length, (i) => List<int>.generate(cachedResult.board[0].length, (index) => cachedResult.board[y][index], growable: false), growable: false);
...
class BoardState {
    ...
    //     board = List<List<int>>.generate(s.board.length, (i) => List<int>.generate(s.board[i].length, (index) => s.board[i][index], growable: false), growable: false);
    //
...
class SlaveThread  { //extends Thread
    ...
    List<List<int>> __board; //board that it's currently working on. a reference is kept here so a copy can be passed to the gui to show the animation. ignore.
